...
function plugin(config) {
	...
	return function(files, metalsmith, done) {
		...
		function CopyS3Objects() {
			...
			async.waterfall([
				function(next) {
					s3.listObjects({Bucket: config['from']}, next);
				},
		...
		function readList(list, next) {
			async.each(list, function(obj, callback) {
				...
				s3.getObject(param).on('success', function(response) {
					...
					if (config['localdir']){
						var localpath = config['localdir'].concat("/").concat(path);
						mkdirp(fspath.dirname(localpath), function (err) {
							if (err) console.error(err)
							else fs.writeFile(localpath, data.Body);
						});
					}
					callback();
				}).send();
